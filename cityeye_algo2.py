...
#===============================================================================
def applyPreprocessing(frame,bgsegm_inst,kern_len=None):
    """
    """    
...
def identifyVehicles(og_frame,stats,centroids,disp_info=None):
    ##NOTE: Between Calls, counter goes to zero, find better solutions
    left_lane_cnt = 0
    right_lane_cnt = 0
    ...
    for idx,obj_xy in enumerate(centroids):
        ...
        cv2.line(og_frame,(120,240),(280,240),(0,255,255),2)
        cv2.line(og_frame,(340,200),(450,200),(0,255,128),2)
        ...
        if stats[idx,4] > 400 and stats[idx,4] < 8000 :
            ...
            if og_frame[int(obj_xy[1]),int(obj_xy[0]),1] == 255 and og_frame[int(obj_xy[1]),int(obj_xy[0]),2] == 255:
                left_lane_cnt = left_lane_cnt+1                    
            if og_frame[int(obj_xy[1]),int(obj_xy[0]),1] == 255 and og_frame[int(obj_xy[1]),int(obj_xy[0]),2] == 128:
            ...
            # Update counter info
            cv2.putText(og_frame,'Left Lane Count:'+str(left_lane_cnt),(20,30),cv2.FONT_HERSHEY_SIMPLEX,0.5,(128,0,255),2)
            cv2.putText(og_frame,'Right Lane Count:'+str(right_lane_cnt),(470,30),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,0,128),2)
    return og_frame
...
fgbg_inst = cv2.bgsegm.createBackgroundSubtractorCNT(minPixelStability=15)
...
while cap.isOpened():
    ret, frame = cap.read()
    if ret == True:        
        prosFrame = applyPreprocessing(frame,fgbg_inst,kern_len=3)
        ObjStat,ObjCentroid = findObjectProps(prosFrame)                      
               
       #Now that we have all the detected components/objects and its stats in a frame we will
       #iterate through each of the detected object and apply rule to get targeted
        for idx,obj_xy in enumerate(ObjCentroid):
            ...
            #try to filter out actual targeted objects(CARS) based on Area, 
            if ObjStat[idx,4] > 400 and ObjStat[idx,4] < 8000 :                
                # Logic to count the vehicles passing through line boundry
